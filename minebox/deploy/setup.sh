...
if [[ -z "$DOMAIN" ]]; then
  echo "DOMAIN is required.  e.g. sudo DOMAIN=minebox.hamaprojects.com bash $0" >&2
  exit 1
fi
if [[ $EUID -ne 0 ]]; then
...
sed -e "s|/opt/minebox|${APP_DIR}|g" \
    -e "s|User=minebox|User=${APP_USER}|" \
    -e "s|Group=minebox|Group=${APP_USER}|" \
    "$APP_DIR/deploy/minebox.service" > /etc/systemd/system/minebox.service
systemctl daemon-reload
systemctl enable --now minebox
sleep 2
systemctl is-active --quiet minebox && echo "    service is running" || {
...
sed -e "s|vps\.yourdomain\.com|${DOMAIN}|g" \
    -e "s|/opt/minebox|${APP_DIR}|g" \
    -e "s|127\.0\.0\.1:3000|127.0.0.1:${PORT}|g" \
    "$APP_DIR/deploy/nginx.conf" > /etc/nginx/sites-available/minebox
ln -sf /etc/nginx/sites-available/minebox /etc/nginx/sites-enabled/minebox
...
enabled_count=$(ls -1 /etc/nginx/sites-enabled 2>/dev/null | grep -cv '^minebox$' || true)
...
if [[ ! -d "/etc/letsencrypt/live/${DOMAIN}" ]]; then
  cat > /etc/nginx/sites-available/minebox <<NGINX
server {
    listen 80;
    listen [::]:80;
    server_name ${DOMAIN};
...
cat > /etc/cron.d/minebox-backup <<CRON
0 3 * * * root APP_DIR=${APP_DIR} ${APP_DIR}/deploy/backup.sh >> /var/log/minebox-backup.log 2>&1
CRON
...
echo "    and restart:  systemctl restart minebox"
echo
