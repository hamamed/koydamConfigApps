 ...
 *
 * Items are stored with the HSL of their dominant garment colour rather than a bucket name, so
 ...
 *
 * `mono` is defined by the absence of saturation rather than by hue — a black texture pack has a
...
export function colorFilterSql(bucketName, paramPrefix = 'color') {
  ...
  if (bucket.monochrome) {
    return {
      sql: `(i.color_sat IS NOT NULL AND i.color_sat <= @${paramPrefix}Sat)`,
  ...
  const hueTest =
    from > to
      ? `(i.color_hue >= @${paramPrefix}From OR i.color_hue < @${paramPrefix}To)`
      : `(i.color_hue >= @${paramPrefix}From AND i.color_hue < @${paramPrefix}To)`;

  return {
    sql: `(i.color_hue IS NOT NULL AND i.color_sat > @${paramPrefix}Sat AND ${hueTest})`,
    params,
  };
}
