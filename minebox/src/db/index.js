...
export const db = new Database(path.join(config.dataDir, 'minebox.db'));
...
export function migrate() {
  ...
  // columns added to the schema after an install would never reach an existing database.
  // Adding them explicitly keeps a deployed instance and a fresh one on the same shape.
  ensureColumn('users', 'platform_id', 'INTEGER');

  // ALTER TABLE cannot add a UNIQUE column in SQLite, so the column goes on plain and the
  // index follows. Both are idempotent.
  db.exec(
