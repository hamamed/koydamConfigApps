...
Three Node services on one VPS, plus the control panel that configures them.
...
`skincraft` has two native modules (`sharp`, `better-sqlite3`); `npm install`
rebuilds them for Apple Silicon on its own.
...
a class to `deploy/shared/css/admin.css` instead. SkinCraft's CSP does allow
inline, and its views use it — that is deliberate, not an oversight.
...
Working and live: single sign-on across all three panels, remote config, ad
