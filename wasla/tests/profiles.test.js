...
test('names differing by case, hamza form, ة/ه or digit script are the same name', () => {
  ...
  assert.ok(profiles.create({ username: 'أمل1', avatar: '🦁' }).token);
  const clash = profiles.create({ username: 'امل١', avatar: '🦊' });
...
test('a profile is found by its token only, which is not stored', () => {
  const { token, profile } = profiles.create({ username: 'sara', avatar: '🦉' });
...
test('stats are checked, the best streak never drops, and badges accumulate', () => {
  ...
  const { profile } = profiles.create({ username: 'sara', avatar: '🦉' });
...
test('boards rank fastest first for times and highest first for points, with my rank beyond the top', () => {
  const players = ['aaa', 'bbb', 'ccc', 'ddd'].map((username, i) => profiles.create({ username, avatar: '🦁' }).profile);
...
test('a streak counts only while it is alive, and banned players leave every board', () => {
  const [alive, stale] = ['alive', 'stale'].map((username) => profiles.create({ username, avatar: '🦁' }).profile);
...
test('deleting a profile removes its scores and badges', () => {
  const { profile } = profiles.create({ username: 'gone', avatar: '🦁' });
...
test('HTTP: create, check, update, post a time, read the board, delete', async () => {
  const created = await call('/profiles', { method: 'POST', body: { username: 'لاعب', avatar: '🐪' } });
  assert.equal(created.status, 201);
  const { token, profile } = await created.json();
  assert.equal(profile.username, 'لاعب');
  assert.ok(token.length >= 40);

  assert.equal((await call('/profiles', { method: 'POST', body: { username: 'لاعب', avatar: '🐪' } })).status, 409);
  ...
  const renamed = await call('/profile/me', { method: 'PATCH', token, body: { username: 'player_1', avatar: '🦉' } });
  ...
  assert.deepEqual(board.entries.map((e) => [e.rank, e.username, e.avatar, e.value, e.isMe]), [[1, 'player_1', '🦉', 240, true]]);
