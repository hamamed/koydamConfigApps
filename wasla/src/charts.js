...
export function barChart(points, { width = 720, height = 240, ticks = 4, xEvery = 5, label = 'Chart' } = {}) {
  ...
  points.forEach((p, i) => {
    ...
    parts.push(`<rect class="wz-chart-bar" x="${x.toFixed(1)}" y="${(margin.top + plotH - h).toFixed(1)}" width="${barW.toFixed(1)}" height="${h.toFixed(1)}" rx="2"><title>${escape(p.title ?? `${p.label}: ${p.value}`)}</title></rect>`);
