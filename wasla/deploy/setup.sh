...
#
#   sudo DOMAIN=wassla.hamaprojects.com EMAIL=you@example.com bash deploy/setup.sh
#
...
# Given its own user, Wasla starts fine and then dies on the first deploy:
# /opt/wasla becomes the shared account\'s, the unit is still wasla\'s, and
...
# owns the directory.
#
# Standalone there is no such account, and Wasla gets its own as before.
if id -u brawl >/dev/null 2>&1; then
  APP_USER="${APP_USER:-brawl}"
else
  APP_USER="${APP_USER:-wasla}"
fi
...
if [[ -z "$DOMAIN" ]]; then
  echo "DOMAIN is required.  e.g. sudo DOMAIN=wassla.hamaprojects.com bash $0" >&2
  exit 1
fi
if [[ $EUID -ne 0 ]]; then
...
mkdir -p "$APP_DIR"/{data,storage/questions,storage/audio}
...
if [[ ! -f "$APP_DIR/package.json" ]]; then
  ...
  Clone the repository on the server and run this from within it:
      ...
      cd /opt/src/koydamConfigApps/wasla
...
DATA_DIR=./data
IMAGES_DIR=./storage/questions
AUDIO_DIR=./storage/audio
PLATFORM_URL=https://config.hamaprojects.com
PLATFORM_APP_SLUG=wasla
SERVICE_TOKEN=
ENV
...
sed -e "s|/opt/wasla|${APP_DIR}|g" \
    -e "s|User=wasla|User=${APP_USER}|" \
    -e "s|Group=wasla|Group=${APP_USER}|" \
    "$APP_DIR/deploy/wasla.service" > /etc/systemd/system/wasla.service
systemctl daemon-reload
systemctl enable --now wasla
sleep 2
systemctl is-active --quiet wasla && echo "    service is running" || {
  echo "    service failed to start — journalctl -u wasla -n 40" >&2
  exit 1
}
...
sed -e "s|vps\.yourdomain\.com|${DOMAIN}|g" \
    -e "s|/opt/wasla|${APP_DIR}|g" \
    -e "s|127\.0\.0\.1:3000|127.0.0.1:${PORT}|g" \
    "$APP_DIR/deploy/nginx.conf" > /etc/nginx/sites-available/wasla
ln -sf /etc/nginx/sites-available/wasla /etc/nginx/sites-enabled/wasla
...
enabled_count=$(ls -1 /etc/nginx/sites-enabled 2>/dev/null | grep -cv '^wasla$' || true)
...
if [[ ! -d "/etc/letsencrypt/live/${DOMAIN}" ]]; then
  cat > /etc/nginx/sites-available/wasla <<NGINX
server {
    listen 80;
    listen [::]:80;
    server_name ${DOMAIN};
...
cat <<SUMMARY
    ...
    API           https://${DOMAIN}/api/v1/levels
    Listening on  127.0.0.1:${PORT} (nginx proxies to it)

SUMMARY

echo "    Sign in with your Koydam account once SERVICE_TOKEN is set in $APP_DIR/.env"
echo "    (sudo /opt/deploy.sh wires it). A local fallback admin:"
echo "      cd $APP_DIR && npm run create-admin -- admin 'a-long-password'"
echo
