...
| Table | Purpose |
| --- | --- |
| `consultations` | open procurement listings, unique on `reference` |
| `consultation_articles` | article / lot breakdown of a consultation |
| `consultation_results` | awards, unique on `reference`, FK `consultation_id` |
| `result_lots` | per-lot award detail (winner, amount, status) |
| `favorites` | `(user_id, consultation_reference)` saved projects |
...
`reference` is the business key shared by both listings. Both parsers normalise
it identically (`AOO 12/2026`, `aoo_12 / 2026` → `AOO12/2026`), so an exact join
is reliable despite the formatting noise in the source HTML.

After each crawl, `matcher.js`:

1. fills `consultation_results.consultation_id` for every reference that exists
   on both sides, stamping `matched_at`;
2. flips the consultation to `has_result = 1`, `status = 'awarded'`;
3. closes consultations whose deadline has passed.

Results whose consultation was never scraped stay unmatched and are reported on
the admin dashboard. Fuzzy matching on `objet` is a deliberate non-goal —
attaching the wrong award to a consultation would corrupt downstream invoices.
...
| `GET` | `/api/consultations/:reference` | detail + articles + award + lots |
| `GET` | `/api/consultations/:reference/articles` | lot breakdown (invoice input) |
| `GET` | `/api/consultations/:reference/result` | matched award |
| `GET` | `/api/results` | filtered awards listing |
| `GET` | `/api/results/:reference` | award detail with per-lot breakdown |
...
| `POST` | `/api/favorites` — `{ reference, note?, tags? }` |
| `DELETE` | `/api/favorites/:reference` |

`GET /api/favorites` accepts the same filter parameters as the main listing, so
the UI can reuse its filter component inside the sub-tab. Favourites store the
*reference*, not a row id, so a saved project keeps resolving to its award even
if the consultation row is re-created by a later scrape.
