...
#
#   sudo DOMAIN=portail.civictrust.ma EMAIL=admin@civictrust.ma bash deploy/setup.sh
#
...
DOMAIN="${DOMAIN:-portail.civictrust.ma}"
EMAIL="${EMAIL:-}"
APP_DIR="${APP_DIR:-/opt/portail}"
SERVICE="portail"
...
if id -u brawl >/dev/null 2>&1; then
  ...
  APP_USER="${APP_USER:-portail}"
fi
# Each app on the box needs its own loopback port: 3000 SkinCraft, 3100 MineBox,
# 3200 Fortnite, 8080 Brawl, 8090 the platform panel. 3300 bons de commande, 3500 is this one's.
PORT="${PORT:-3500}"
SKIP_TLS="${SKIP_TLS:-0}"
...
install -m 644 "$APP_DIR/deploy/portail.service"        /etc/systemd/system/portail.service
...
Next:
  ...
  1. Create the first account — this is now the only place a password is set
     for any CivicTrust service:
       cd ${APP_DIR}
       sudo -u ${APP_USER} node bin/users.js add you@civictrust.ma '<a long password>' --role=admin
