...
#
#   sudo DOMAIN=admin.civictrust.ma EMAIL=admin@civictrust.ma bash deploy/setup.sh
#
...
DOMAIN="${DOMAIN:-admin.civictrust.ma}"
EMAIL="${EMAIL:-}"
APP_DIR="${APP_DIR:-/opt/admin}"
SERVICE="admin"
...
# 3200 Fortnite, 8080 Brawl, 8090 the platform panel. 3300 bons de commande, 3600 is this one's.
PORT="${PORT:-3600}"
SKIP_TLS="${SKIP_TLS:-0}"
...
install -d -o "$APP_USER" -g "$APP_USER" "$APP_DIR" "$APP_DIR/storage/invoices"
...
install -m 644 "$APP_DIR/deploy/admin.service"        /etc/systemd/system/admin.service
...
Next:
  ...
  1. Give this console the SAME JWT_SECRET as portail, bdc and marches. It
     verifies their sessions and signs none of its own:
       grep -E '^JWT_SECRET=' ${APP_DIR}/.env
NOTE
