...
/**
 * The origins this console posts forms to and links out at, for the CSP.
 *
 * `form-action` is enforced across redirects, so an origin missing from this
 * list shows up as a form that submits and then goes nowhere — the console's
 * own forms all post back here, but the list is built from config so a service
 * added later does not have to be remembered twice.
 */
function serviceOrigins() {
  const origins = new Set([config.auth.portalUrl])
  for (const service of config.services) {
    for (const url of [service.api, service.panel]) {
      try {
